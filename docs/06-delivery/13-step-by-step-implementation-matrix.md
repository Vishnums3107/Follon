...
**Reconciled snapshot: 2026-08-17.** This is the execution order for work
...
| 1 | Explicit execution prices - backtesting capability | Complete 2026-08-17 | Full spread pays/concedes one half-spread, slippage is adverse, fees are exact, and post-cost limit prices are enforced; configuration remains content-addressed |
...
| 9 | Commercial production gate - roadmap Months 18-20 | Local evidence primitives implemented; external platform work pending | Customer identity/authorization, payment provider, production key custody, legal/security operations, penetration testing, and 10 professionals or 3 organizations remain |
...
- Borrow modeling is not silently approximated. The current research ledger is
  long-only and rejects short positions, so borrow availability, recalls, and
  fees become required only with a separately approved short-selling scope.
...
- The effective-date boundary prevents post-delist replay bars from reaching a
  strategy. Economic delisting settlement, liquidation, and recovery-value
  policies remain required before researching instruments whose outcome depends
  on those events.
